...
- Use Cajita tables to mark Ref objects so that Cajita objects can't spoof Ref behavior.
- Review robustness of ejector-invoked-out-of-scope behavior.
- Make all E objects (generated by compiler and by primitive library) be Cajita-frozen.
- Add robustness against mid-turn-exited turns, such as in hang timeouts: set a 'currently in turn' flag which is cleared in a finally block, and fail if the flag is already set.

Completeness:
- Implement Equalizer, maps (as well as the rest of the standard library).
- Get it running standalone, so we can have a REPL and animateUpdoc entirely within the browser:
...
- Better updoc-running facilities:
  ...
  - When converting an Updoc-in-HTML file, do a better job of preserving the original markup.
- Have JavaScript code use 'Foo' rather than 'makeFoo' for constructor functions because that is JavaScript idiom.
- Make E functions (anything with a run/N verb) be JS functions, if there is no downside of this.
